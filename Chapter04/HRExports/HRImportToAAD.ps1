...
(get-content "$($dir)\NewHire.csv") | foreach-object {$_ -replace "contoso.com" , $domain.Name} | Set-Content "$($dir)\NewHire.csv"import-csv "$($dir)\NewHire.csv" | foreach {New-Msoluser -userPrincipalName $_.UserPrincipalName -displayname $_.displayname -firstname $_.firstname -lastname $_.lastname -password $_.Password -mobilephone $phone -forcechangepassword $false -usagelocation "CH" | set-msoluserlicense -addlicenses "$($domain.name -replace ".onmicrosoft.com" ,":EMS")","$($domain.name -replace ".onmicrosoft.com" , ":ENTERPRISEPACK")"}
